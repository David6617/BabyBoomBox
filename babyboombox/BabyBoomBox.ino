/*
 * BabyBoomBox - Keypad → WAV playback on buzzer (16 buttons, 16 sounds in flash)
 *
 * No SD card. WAV data is embedded in the sketch (see wav_data.h).
 * Library: Keypad only.
 *
 * --- WIRING ---
 * Buzzer → Arduino: Buzzer + → Pin 10, Buzzer - → GND
 * Keypad 4×4: Rows 9,8,7,6; Cols 5,4,3,2
 *
 * --- ADDING YOUR WAV FILES ---
 * Run tools/wav_to_header.py to convert 001.wav..016.wav into wav_data.h,
 * then put wav_data.h in the same folder as this sketch.
 * WAV must be: 8-bit unsigned PCM, mono, 8 kHz (script can resample).
 */
...
// Key → WAV slot index 0..15 (001..016). Returns 255 if unknown.
...
// Play 8-bit PCM from PROGMEM at SAMPLE_RATE_HZ. Blocking.
void playFromProgmem(const uint8_t* data, unsigned int length) {
  ...
  for (unsigned int i = 0; i < length; i++) {
    ...
    analogWrite(SPEAKER_PIN, s >> 1);  // 8-bit to 7-bit PWM range
